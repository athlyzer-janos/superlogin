...
module.exports = function(config) {
  ...
  this.sendEmail = function(templateName, email, locals) {
    ...
    var body = "";
    if (config.getItem('emails.header')) {
      let tHeader = fs.readFileSync(config.getItem('emails.header.template'), 'utf8');
      body += ejs.render(tHeader, {});
    }
    if (typeof config.getItem('emails.i18nFunction') == "function") {
      locals.__ = function(phrase, locale) {
        if (!locale) {
          locale = "de";
        }
        return config.getItem('emails.i18nFunction')({
          phrase: phrase,
          locale: locale
        })
      };
      body += ejs.render(template, locals);
    } else {
      body += ejs.render(template, locals);
    }
    ...
    if (config.getItem('emails.footer')) {
      let tFooter = fs.readFileSync(config.getItem('emails.footer.template'), 'utf8');
      body += ejs.render(tFooter, {});
    }
    // form the email
    var subject = config.getItem('emails.' + templateName + '.subject');

    if (typeof config.getItem('emails.i18nFunction') == "function" && locals.user && locals.user.sprache) {
      subject = config.getItem('emails.i18nFunction')({phrase : subject, locale : locals.user.sprache || "de"});
    ...
    // send the message
    var sendEmail = BPromise.promisify(transporter.sendMail, {
      context: transporter
    });
