...
module.exports = function(config, router, passport, user) {
  ...
  router.post('/login', function(req, res, next) {
    ...
    try {
      await user.myLoginAction(req.user,req);
      return user.createSession(req.user._id, 'local', req)
        .then(async function (mySession) {
          // newSession = await mySessionAction();
          console.log("createSession");
          mySession = await user.mySessionAction(mySession, req.user,req);
      ...
      let message   =   e.message || "error at login";
  ...
  router.post('/refresh',
    ...
    function (req, res, next) {
      return user.refreshSession(req.user.key)
        .then(async function (mySession) {
          console.log("updateSession",req.user);
          mySession = await user.mySessionAction(mySession, req.user,req);
          res.status(200).json(mySession);
        }, function (err) {
          return next(err);
