...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  var onLoginActions = [];
  var onLinkActions = [];
  var onSessionAction = function(hash, resolveProperty){
    return Promise.resolve(hash[resolveProperty]);
  };
  ...
  this.onLogin = function(fn){
    if(typeof fn === 'function') {
      onLoginActions.push(fn);
    } else {
      throw new TypeError('onLogin: You must pass in a function');
    }
  }

  this.onSession = function(fn){
    if(typeof fn === 'function') {
      onSessionAction = fn;
    } else {
      throw new TypeError('onSession: You must pass in a function');
    }
  }

  this.myLoginAction = function(userDoc,req){
    return processTransformations(onLoginActions, userDoc, 'local', req);
  }


    this.mySessionAction = function(mySession, userDoc,req){
      return processTransformation(onSessionAction, { "userDoc" : userDoc, "session" : mySession, "req" : req},"session");
  ...
  function processTransformation(fn, hash, resolveProperty) {
    var promise;
      if(!promise) {
        console.log("i am here");

        promise = fn.call(null, hash,resolveProperty);
      } else {
        if(!promise.then || typeof promise.then !== 'function') {
          throw new Error('onCreate function must return a promise');
        }
        promise.then(function(newUserDoc) {
          return fn.call(null, hash,resolveProperty);
        });
      }
    if(!promise) {
      if(resolveProperty){
          promise = BPromise.resolve(hash[resolveProperty]);
      }else{
          promise = BPromise.resolve(hash);
      }
    }
    return promise;
  }

  function processTransformations(fnArray, userDoc, provider,req) {
    var promise;
    fnArray.forEach(function(fn) {
      if(!promise) {
        promise = fn.call(null, userDoc, provider,req);
      } else {
        if(!promise.then || typeof promise.then !== 'function') {
          throw new Error('onCreate function must return a promise');
        }
        promise.then(function(newUserDoc) {
          return fn.call(null, newUserDoc, provider,req);
        });
      }
    });
    if(!promise) {
  ...
  this.create = function (form, req) {
    ...
    return user.process()
      ...
      .then(function(newUser) {
        return processTransformations(onCreateActions, newUser, 'local',req);
      })
  ...
  this.socialAuth = function(provider, auth, profile, req) {
    ...
    return BPromise.resolve()
      ...
      .then(function(userDoc) {
        if(newAccount) {
          return processTransformations(onCreateActions, userDoc, provider,req);
        } else {
          return processTransformations(onLinkActions, userDoc, provider,req);
        }
      })
  ...
  this.createOneTimeSession = function(dbname){

    console.log("createOneTimeSession",dbname);
    const cloudantApi = require('./dbauth/cloudant');
    let getAPIKey = cloudantApi.getAPIKey({ name : dbname });
    return getAPIKey.then(async function(key){
      console.log("getAPIKey then key",key);
      let permission = await cloudantApi.authorizeKeys(undefined,{ name : dbname } ,key.key,["_reader"]);
